...
contactusDisplayControllers.controller('ContactusDisplayCtrl', ['$scope', 'ContactType', 'GetAllContacts', '$http', 
	 function($scope, ContactType, $http) {
	   ...
	   $scope.locationMarkers = GetAllContacts.query();
	   $scope.location=""; 
	   $scope.contactType="";
	   ...
	   $scope.loadReportByContactType = function(contactType) {
		    ...
		    success(function(data, status, headers, config) {
		      $scope.locationMarkers = angular.fromJson(data.displaycontacts);
		    }).
	  ...
	  $scope.loadReportByLocation = function(locationInfo) {
	    ...
	    $scope.loc = { lat: 37.501777, lon: 15.074518 };
...
contactusDisplayControllers.filter('lat', function () {
...
contactusDisplayControllers.filter('lon', function () {
    return function (input, decimals) {
        if (!decimals) decimals = 0;
        input = input * 1;
        var ew = input > 0 ? "E" : "W";
...
contactusDisplayControllers.directive("appMap", function () {
    return {
        restrict: "E",
        replace: true,
        template: "<div></div>",
